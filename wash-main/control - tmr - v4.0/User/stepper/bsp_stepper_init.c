...
static void Stepper_GPIO_Config(void) 
{
	...
	//GPIO_InitStruct. =GPIO_PULLUP;
	/*设置引脚速率为高速 */   
	GPIO_InitStruct.GPIO_Speed = GPIO_Speed_50MHz;
	/*Motor 方向引脚 初始化*/
	GPIO_Init(MOTOR_DIR_GPIO_PORT, &GPIO_InitStruct);	

	/*Motor 使能引脚 初始化*/
	GPIO_InitStruct.GPIO_Pin = MOTOR_EN_PIN;	
	GPIO_Init(MOTOR_EN_GPIO_PORT, &GPIO_InitStruct);	
	...
	/*选择要控制的GPIO引脚*/	
	GPIO_InitStruct.GPIO_Pin = MOTOR_PUL_PIN;
	/*Motor 脉冲引脚 初始化*/
	GPIO_Init(MOTOR_PUL_PORT, &GPIO_InitStruct);			
}
